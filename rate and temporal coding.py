...
# ---------- PARAMETERS ----------
USE_WEBCAM = True  # promijeni na True za lokalnu kameru (ako je dostupna)
...
def events_to_spiketrains(events_sequence, window=SPIKE_TIME_WINDOW, mode="temporal"): # ovo mijenjamo rate/temporal
    """
    Pretvara niz event mapa u spike train.
    mode:
      - "rate": event -> spike odmah (1:1 mapping)
      - "temporal": jačina promjene -> vrijeme spike-a unutar prozora
    """
    ...
    if mode == "rate":
        # jednostavno: spike gdje ima event
        for t, ev in enumerate(events_sequence):
            spikes[t] = (ev != 0).astype(np.uint8)

    elif mode == "temporal":
        # intenzitet promjene (|diff|) -> spike time unutar windowa
        for t, ev in enumerate(events_sequence):
            mag = np.abs(ev)
            # normaliziraj magnitude u [0,1]
            norm = np.clip(mag / (np.max(mag) + 1e-6), 0, 1)
            # mapiraj na delay (0 = odmah, 1 = kasno)
            delays = (1 - norm) * (window - 1)
            for y in range(H):
                for x in range(W):
                    if ev[y, x] != 0:
                        delay = int(delays[y, x])
                        spike_time = min(t + delay, T - 1)
